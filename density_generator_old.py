...
def save_computed_density_csv(density_map, out_path):
    np.savetxt(out_path, density_map, delimiter=',')
...
def get_gt_dots(mat_path, img_height, img_width):
    dataframe = pandas.read_csv(mat_path, header=None)
    gt = dataframe.values.astype(np.float).round().astype(int)
...
def compute_distances(out_dist_path='distances_dict.pkl', Shanghai_root_path='./', n_neighbors=4, leafsize=1024):
    distances_dict = dict()
    full_img_pathes = glob.glob(f'{Shanghai_root_path}/*/*/images/*.png')

    for full_img_path in tqdm(full_img_pathes):
        mat_path = full_img_path.replace('.png', '.csv').replace('images', 'ground-truth')
        if os.path.isfile(mat_path) == False:
            continue
...
generate_gaussian_kernels(precomputed_kernels_path, round_decimals=3, sigma_threshold=4, sigma_min=0, sigma_max=8,
                          num_sigmas=801)
...
compute_distances(out_dist_path=precomputed_distances_path,
                  Shanghai_root_path='data/ShanghaiTech')

with open(precomputed_distances_path, 'rb') as f:
    distances_dict = pickle.load(f)

# generate GT for part A
data_root = 'data/ShanghaiTech\\part_A_standardized/'
img_pathes = glob.glob(f'{data_root}*/images/*.png')
...
for full_img_path in tqdm(img_pathes):
    ...
    exists = os.path.isfile(mat_path)
    density_map = None
    if exists == False:
    ...
    gt_out_path = curr_map_out_folder + img_path.strip('/').replace('.png', '.csv')

    #plt.imshow(img)
    #plt.imshow(density_map, alpha=.75)
    #plt.show()

    if not os.path.isdir(curr_map_out_folder):
        print('creating ' + curr_map_out_folder)
        os.makedirs(curr_map_out_folder)
    save_computed_density_csv(density_map, gt_out_path)
    print(gt_out_path)
